...
def save_probe_and_results(
    ...
    # Save probe model
    probe_path = os.path.join(probe_dir, f"{dataset_name}_layer{layer_num}_probe.pt")
    ...
    json_results = {
        ...
        "losses": (
            [float(loss) if hasattr(loss, "item") else loss for loss in losses]
            if losses
            else None
        ),
    }
...
def plot_accuracies_by_layer(
    save_dir: str, model_name: str, dataset_name: str = "mixed"
):
    ...
    mixed_accs = [
        results[dataset_name][layer]["results"]["mixed"]["accuracy"] for layer in layers
    ]
    mixed_ci_lower = [
        results[dataset_name][layer]["results"]["mixed"]["ci_lower"] for layer in layers
    ]
    mixed_ci_upper = [
        results[dataset_name][layer]["results"]["mixed"]["ci_upper"] for layer in layers
    ]

    pol_accs = [
        results[dataset_name][layer]["results"]["politics"]["accuracy"]
        for layer in layers
    ]
    pol_ci_lower = [
        results[dataset_name][layer]["results"]["politics"]["ci_lower"]
        for layer in layers
    ]
    pol_ci_upper = [
        results[dataset_name][layer]["results"]["politics"]["ci_upper"]
        for layer in layers
    ]
    ...
    print("\nSummary Statistics:")
    print(
        f"Mixed Dataset - Mean Accuracy: {np.mean(mixed_accs):.3f} (95% CI: {np.mean(mixed_ci_lower):.3f}-{np.mean(mixed_ci_upper):.3f})"
    )
    print(
        f"Politics Dataset - Mean Accuracy: {np.mean(pol_accs):.3f} (95% CI: {np.mean(pol_ci_lower):.3f}-{np.mean(pol_ci_upper):.3f})"
    )
    ...
    print(
        f"Best Layer for Politics: {layers[best_pol_idx]} (Acc: {max(pol_accs):.3f}, CI: {pol_ci_lower[best_pol_idx]:.3f}-{pol_ci_upper[best_pol_idx]:.3f})"
    )
