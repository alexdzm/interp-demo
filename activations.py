...
def collect_and_save_activations(
    ...
    def collect_activations(dataloader, split_name):
        ...
        with torch.no_grad():
            for batch in tqdm(dataloader, desc=f"Collecting activations for {dataset_name} ({split_name})"):
        ...
        save_path = os.path.join(save_dir, f"{model.cfg.model_name}/{dataset_name}_{split_name}_activations.pkl")
...
def load_activation_dataloaders(
    ...
    def load_split(split_name):
        save_path = os.path.join(save_dir, f"{model_name}/{dataset_name}_{split_name}_activations.pkl")
